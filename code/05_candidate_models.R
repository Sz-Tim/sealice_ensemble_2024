...
for(i in 1:nrow(sim_i)) {
  sim <- sim_i$sim[i]
  dat_rstan <- make_data_rstan(fit_df |> select(-any_of(sim_i$sim[-i]), -any_of(paste0("c_", sim_i$sim[-i]))))
  ...
  saveRDS(out_sim, glue("out/candidates/{sim}_RE_stanfit.rds"))
}
...
for(i in 1:length(sim_avgs)) {
  sim <- sim_avgs[i]
  dat_rstan <- make_data_rstan(fit_df |> select(-any_of(sim_avgs[-i]), -any_of(paste0("c_", sim_avgs[-i]))))
  out_sim <- stan(file="code/stan/candidate_model_ranef.stan",
                  model_name=sim,
                  data=dat_rstan,
                  chains=6, cores=6,
                  iter=3000, warmup=2500,
                  pars=c("b_b0", "b_IP", "Intercept_hu", "b_hu", "sigma", "r_grp_hu", "sd_grp_hu"))
  saveRDS(out_sim, glue("out/ensembles/{sim}_RE_stanfit.rds"))
}
