...
data {
  ...
  matrix[N, K_sims] X_p;  // design matrix
  
  // data for spline 1 
  int nb_pMix02_1;  // number of bases = K_sims?
  array[nb_pMix02_1] int knots_pMix02_1;  // number of knots
  // basis function matrices // matrices created by mgcv::smoothCon I think
  matrix[N, knots_pMix02_1[1]] Zs_pMix02_1_1;
...
transformed data {
  ...
  matrix[N_Yg0, K_sims] X_p_Yg0 = X_p[indexes_Yg0,];
  ...
  matrix[N, K_int_sims] X_hu = append_col(rep_vector(1.0, N), X_p);
  matrix[N, K_sims] Xc_hu;  // centered version of X_hu without an intercept
  vector[K_sims] means_X_hu;  // column means of X_hu before centering
  for (i in 2:K_int_sims) {
...
parameters {
  ...
  vector<lower=0,upper=1>[K_sims] bs_pMix02;  // unpenalized spline coefficients
  // parameters for spline 1
  // standardized penalized spline coefficients
  vector[knots_pMix02_1[1]] zs_pMix02_1_1;
  vector<lower=0>[nb_pMix02_1] sds_pMix02_1;  // SDs of penalized spline coefficients
...
transformed parameters {
  ...
  vector[knots_pMix02_1[1]] s_pMix02_1_1;
  ...
  s_pMix02_1_1 = sds_pMix02_1[1] * zs_pMix02_1_1;
  ...
  lprior += normal_lpdf(bs_pMix02 | 0.5, 1)
    - 2 * log_diff_exp(normal_lcdf(1 | 0.5, 1), normal_lcdf(0 | 0.5, 1));
  lprior += student_t_lpdf(sds_pMix02_1 | 3, 0, 2.5)
...
model {
  ...
  if (!prior_only) {
    ...
    nlp_pMix02 += X_pMix02 * b_pMix02 + X_p_Yg0 * bs_pMix02 + Zs_pMix02_1_1 * s_pMix02_1_1;
    
    hu = Intercept_hu + Xc_hu * b_hu;
    mu = b_b0 + b_IP * (X_p_Yg0 * b_p);
